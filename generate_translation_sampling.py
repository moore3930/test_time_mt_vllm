...
def run_batch_translation(
    ...
    if decoding != "sampling":
        raise ValueError("generate_translation_sampling.py only supports --decoding sampling")
    if num_rounds <= 0:
    ...
    for chunk in batched(items, batch_size):
        ...
        for item, output in zip(chunk, outputs):
            ...
            round_texts = [candidate.text.strip() for candidate in output.outputs]
            if len(round_texts) != num_rounds:
                raise RuntimeError(
                    f"Expected {num_rounds} samples per input, got {len(round_texts)} for id={item['id']}"
                )
