...
def _normalize_sentence_list(values: object) -> List[str]:
    if isinstance(values, list):
        return [str(x).strip() for x in values]
    return []


def _split_doc_text_to_sentences(text: object) -> List[str]:
...
def _align_sentence_count(lines: List[str], expected: int) -> List[str]:
    if expected <= 0:
        return lines
    ...
    if len(lines) > expected:
        ...
        merged.append(" ".join(lines[expected - 1 :]).strip())
        return merged
    return lines + [""] * (expected - len(lines))


def _get_mt_sentences(row: Dict, mt_key: str, expected_count: int) -> List[str]:
    sentence_key = f"{mt_key}_sentences"
    sentence_list = _normalize_sentence_list(row.get(sentence_key))
    if not sentence_list:
        sentence_list = _split_doc_text_to_sentences(row.get(mt_key))
    return _align_sentence_count(sentence_list, expected_count)


def _can_do_sentence_level(rows: List[Dict], mt_key: str, needs_ref: bool) -> bool:
    if not rows:
        return False
    for row in rows:
        src_sentences = _normalize_sentence_list(row.get("source_sentences"))
        if not src_sentences:
            return False
        if mt_key not in row:
            return False
        if needs_ref and not _normalize_sentence_list(row.get("target_sentences")):
            return False
    return True


def _score_key_for_rows_sentence_level(
    rows: List[Dict],
    mt_key: str,
    score_key: str,
    needs_ref: bool,
    ...
    for row_idx, row in enumerate(rows):
        src_sentences = _normalize_sentence_list(row.get("source_sentences"))
        mt_sentences = _get_mt_sentences(row, mt_key, len(src_sentences))
        if needs_ref:
            ref_sentences = _align_sentence_count(_normalize_sentence_list(row.get("target_sentences")), len(src_sentences))
        for sent_idx, (src_sent, mt_sent) in enumerate(zip(src_sentences, mt_sentences)):
            if needs_ref:
                inputs.append({"src": src_sent, "mt": mt_sent, "ref": ref_sentences[sent_idx]})
            else:
                inputs.append({"src": src_sent, "mt": mt_sent})
            index_map.append((row_idx, sent_idx))

    pred = model.predict(inputs, batch_size=score_batch_size, gpus=comet_gpus, progress_bar=True)
    scores = _extract_scores(pred)
    if len(scores) != len(index_map):
        raise RuntimeError("Sentence-level score count mismatch with flattened rows.")

    for score, (row_idx, _sent_idx) in zip(scores, index_map):
        per_row_scores[row_idx].append(float(score))

    for row, sentence_scores in zip(rows, per_row_scores):
        if not sentence_scores:
            row[f"{score_key}_sentences"] = []
            row[score_key] = 0.0
            continue
        row[f"{score_key}_sentences"] = sentence_scores
        row[score_key] = sum(sentence_scores) / len(sentence_scores)
...
def _score_key_for_rows(
    ...
    if _can_do_sentence_level(rows, mt_key, needs_ref):
        _score_key_for_rows_sentence_level(
            rows=rows,
            mt_key=mt_key,
            score_key=score_key,
            needs_ref=needs_ref,
