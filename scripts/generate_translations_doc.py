    ...
    return [
        {
            ...
            "content": (
                ...
                "Translate line by line and preserve line breaks exactly."
            ),
        },
        {
            "role": "user",
            "content": (
                f"Translate the following document from {src_lang} to {tgt_lang}.\n"
                "Each input line is one sentence.\n"
                "Output ONLY the translation with the same number of lines and the same line order.\n\n"
                f"{text}"
            ),
        },
    ]
...
def load_doc_jsonl_inputs(
    dataset_root: str,
    lang_pair: str,
    max_samples: int,
) -> List[Dict]:
    pair_dir = Path(dataset_root) / lang_pair
    input_path = pair_dir / f"test.{lang_pair}.jsonl"
    ...
    for idx, line in enumerate(input_path.read_text(encoding="utf-8").splitlines()):
        ...
        if not line:
            continue
        row = json.loads(line)
        src_sentences = [x.strip() for x in row.get("source_sentences", []) if isinstance(x, str) and x.strip()]
        ref_sentences = [x.strip() for x in row.get("target_sentences", []) if isinstance(x, str) and x.strip()]
...
def split_translation_lines(text: str, expected_lines: int) -> List[str]:
    cleaned = text.strip()
    if not cleaned:
        return [""] * max(expected_lines, 0)
    ...
    if len(lines) == expected_lines:
        return lines
    if len(lines) > expected_lines:
        merged = lines[: expected_lines - 1]
        merged.append(" ".join(lines[expected_lines - 1 :]).strip())
        return merged
    return lines + [""] * (expected_lines - len(lines))
...
def shuffle_document_with_ratio(
    text: str,
    noise_ratio: float,
    lang_key: str,
    tokenizer: AutoTokenizer,
...
def run_batch_translation(
    ...
    for chunk in batched(items, batch_size):
        messages_per_item: List[List[Dict[str, str]]] = [
            build_first_turn_messages(item["source_doc"], src_lang, tgt_lang) for item in chunk
        ]
        round_predictions_per_item: List[List[str]] = [[] for _ in chunk]
        round_sentence_predictions_per_item: List[List[List[str]]] = [[] for _ in chunk]
        round_noised_per_item: List[List[str]] = [[] for _ in chunk]
        for round_idx in range(1, num_rounds + 1):
            ...
            for i, text in enumerate(round_texts):
                translated_lines = split_translation_lines(
                    text=text,
                    expected_lines=len(chunk[i]["source_sentences"]),
                )
                canonical_doc_text = "\n".join(translated_lines)
                round_predictions_per_item[i].append(canonical_doc_text)
                round_sentence_predictions_per_item[i].append(translated_lines)
                history_text = shuffle_document_with_ratio(
                    ...
                    messages_per_item[i].append(
                        {
                            "role": "user",
                            "content": (
                                "Please translate the same source text again and improve quality. "
                                "Keep the same number of lines and output only the translation."
                            ),
                        }
                    )

        for item, round_texts, round_sentence_texts, noised_round_texts in zip(
            chunk, round_predictions_per_item, round_sentence_predictions_per_item, round_noised_per_item
        ):
            row = dict(item["raw_row"])
            row["id"] = item["id"]
            row["source_text"] = item["source_doc"]
            row["reference_text"] = item["reference_doc"]
            for i, text in enumerate(round_texts, start=1):
                row[f"hypo_{i}"] = text
                row[f"hypo_{i}_sentences"] = round_sentence_texts[i - 1]
            for i, text in enumerate(noised_round_texts, start=1):
                row[f"noise_hypo_{i}"] = text
            results.append(row)
    return results
...
def main() -> None:
    ...
    parser.add_argument("--dataset-root", default="datasets/wmt24pp_doc")
    ...
    for lang_pair in lang_pairs:
        ...
        items = load_doc_jsonl_inputs(args.dataset_root, lang_pair, args.max_samples)
        if not items:
