...
def run_batch_translation(
    ...
    if branch_factor <= 0:
        raise ValueError("--branch-factor must be > 0")
    if decoding == "beam_search" and branch_factor > 1:
        raise ValueError("--branch-factor > 1 is not supported with beam_search")
    ...
    for chunk in batched(items, batch_size):
        for item in chunk:
            root_messages = build_first_turn_messages(item["source_text"], src_lang, tgt_lang)
            ...
            for round_idx in range(1, num_rounds + 1):
                prompts = [
                    render_chat_prompt(tokenizer, node["messages"], disable_thinking=disable_thinking)
                    for node in active_nodes
                ]
                outputs = llm.generate(prompts, sampling)

                next_nodes: List[Dict] = []
                round_nodes: List[Dict] = []
                for parent_node, output in zip(active_nodes, outputs):
                    samples = [candidate.text.strip() for candidate in output.outputs]
                    if not samples:
                        samples = [""]
                    for choice_idx, text in enumerate(samples, start=1):
                        ...
                        node_counter += 1
                        node_id = f"r{round_idx}_n{node_counter}"
                        round_nodes.append(
                            {
                                "node_id": node_id,
                        ...
                        child_messages = [*parent_node["messages"], {"role": "assistant", "content": history_text}]
                        if round_idx < num_rounds:
                            child_messages.append(
                                {
                                    "role": "user",
                                    "content": "Please translate again for a better version.",
                                }
                            )
                        next_nodes.append(
                            {
            ...
            for i, node in enumerate(active_nodes, start=1):
                row[f"hypo_{i}"] = node["hypothesis"]
                row[f"noise_hypo_{i}"] = node["history_text"]

            row["tree"] = {
                "branch_factor": branch_factor,
                "num_rounds": num_rounds,
                "num_leaf_hypotheses": len(active_nodes),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of children generated from each node at each round.",
    )
    parser.add_argument(
        "--history-noise-ratio",
        type=float,
    ...
    for lang_pair in lang_pairs:
        ...
        meta = {
            ...
            "num_rounds": args.num_rounds,
            "branch_factor": args.branch_factor,
            "leaf_hypotheses_per_row": args.branch_factor ** args.num_rounds,
            "history_noise_ratio": args.history_noise_ratio,
            "model": args.model,
            "decoding": args.decoding,
            "temperature": args.temperature,
